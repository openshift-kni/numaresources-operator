...
//+kubebuilder:rbac:groups=apps,resources=deployments,verbs=create;list;watch,namespace="numaresources"
//+kubebuilder:rbac:groups=apps,resources=deployments,resourceNames=secondary-scheduler,verbs=get;update,namespace="numaresources"
...
func (r *NUMAResourcesSchedulerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.ImageValidation.Enabled {
		if err := validation.SchedulerImage(instance.Spec.SchedulerImage, r.ImageValidation.Digests); err != nil {
			return ctrl.Result{}, r.degradeStatus(ctx, initialStatus, instance, status.ReasonInvalidSchedulerImage, err.Error())
		}
	} else {
