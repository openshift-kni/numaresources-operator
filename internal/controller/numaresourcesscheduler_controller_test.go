...
var _ = ginkgo.Describe("Test NUMAResourcesScheduler Reconcile", func() {
	...
	ginkgo.Context("with kubelet PodResourcesAPI listing active pods by default", func() {
		...
		ginkgo.When("kubelet fix is enabled", func() {
			...
			ginkgo.DescribeTable("should configure by default the informerMode to the expected when field is not set", func(reconcilerPlatInfo PlatformInfo) {
				var err error
				nrs = testobjs.NewNUMAResourcesScheduler("numaresourcesscheduler", "some/url:latest", testSchedulerName, 11*time.Second)
				initObjects := []runtime.Object{nrs}
				initObjects = append(initObjects, fakeNodes(numOfMasters, 3)...)
				...
				_, err = reconciler.Reconcile(context.TODO(), reconcile.Request{NamespacedName: key})
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
				expectedInformer := depmanifests.CacheInformerDedicated
				if reconcilerPlatInfo.Platform == platform.OpenShift || reconcilerPlatInfo.Platform == platform.HyperShift {
					expectedInformer = depmanifests.CacheInformerShared
				}
				...
				ginkgo.Entry("with unknown platform the default informer is Dedicated (unchanged)", PlatformInfo{}))
