	...
	Context("with at least two nodes suitable", Label(label.Tier0), func() {
		...
		DescribeTable("a pod should be placed and aligned on the node", Label(label.Tier0, "hostlevel"),
			func(tmPolicy string, requiredRes []corev1.ResourceList, expectedQOS corev1.PodQOSClass) {
				...
				fxt.Dump.Infof(intreslist.ToString(intreslist.FromContainerRequests(pod.Spec.Containers)), "pod resources for %s/%s", updatedPod.Namespace, updatedPod.Name)
		...
		DescribeTable("[hostlevel][failalign] a pod with multi containers should be pending due to unavailable host-level resources", Label("hostlevel", "failalign"), Label("feature:unsched"),
			func(tmPolicy string, requiredRes []corev1.ResourceList, expectedQOS corev1.PodQOSClass) {
				...
				for nodeName := range candidateNodeNames {
					...
					fxt.Dump.Infof(intreslist.ToString(paddingResources), "padding node %s", nodeName)
