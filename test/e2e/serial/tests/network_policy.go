...
// - Ingress traffic to the metrics endpoints is allowed (note: this is already tested indirectly in metrics.go and will not be re-tested here).
//
...
var _ = Describe("network policies are applied", Ordered, Label("feature:network_policies"), func() {
	...
	var operatorPod, schedulerPod, rteWorkerPod, networkOperatorPod *corev1.Pod
	...
	BeforeAll(func() {
		...
		networkOperatorPods, err := e2eclient.K8sClient.CoreV1().Pods("openshift-network-operator").List(ctx, metav1.ListOptions{
			LabelSelector: labels.SelectorFromSet(map[string]string{
				"name": "network-operator",
			}).String(),
		})
		Expect(err).ToNot(HaveOccurred())
		Expect(networkOperatorPods.Items).ToNot(BeEmpty())
		networkOperatorPod = &networkOperatorPods.Items[0]
	})
	...
	DescribeTable("traffic behavior",
		...
		// Testing network traffic restrictions between pods in the numaresources namespace
		...
		// Testing network traffic restrictions between pods cross namespaces (numaresouces and openshift-network-operator)
		Entry("numaresouces operator -> network operator", trafficCase{
			FromPod:     func() *corev1.Pod { return operatorPod },
			ToHost:      func() string { return networkOperatorPod.Status.PodIP },
			ToPort:      "9104",
			ShouldAllow: false,
			Description: "numaresources operator should NOT access network operator pod",
		}),

		Entry("network operator -> numaresouces operator", trafficCase{
			FromPod:     func() *corev1.Pod { return networkOperatorPod },
			ToHost:      func() string { return operatorPod.Status.PodIP },
			ToPort:      "8081", // Liveness probe
			ShouldAllow: false, 
			Description: "network operator pod should NOT access numaresources operator pod",
		}),
	)
})
...
func trafficTest(cli *kubernetes.Clientset, ctx context.Context, sourcePod *corev1.Pod, destinationIP, destinationPort string) bool {
	...
	cmd := []string{"sh", "-c", fmt.Sprintf("curl -k --connect-timeout 5 http://%s", endpoint)}
