...
var _ = Describe("[serial][scheduler][cache] scheduler cache stall", Label("scheduler", "cache", "stall"), Label("feature:cache", "feature:stall"), func() {
	...
	Context("using a NodeGroup with periodic unevented updates", Label("periodic_update", label.OpenShift), func() {
		...
		When("there are jobs in the cluster", Label("job", "generic", label.Tier0), func() {
			...
			It("should be able to schedule pods with no stalls (vs best-effort)", func() {
			...
			It("should be able to schedule pods with no stalls (vs guaranteed)", func() {
				...
				klog.Infof("Creating a job whose containers require %q", e2ereslist.ToString(jobRequiredRes))
