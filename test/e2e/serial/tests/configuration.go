...
var _ = Describe("[serial][disruptive] numaresources configuration management", Serial, Label("disruptive"), Label("feature:config"), func() {
	...
	Context("cluster has at least one suitable node", func() {
		...
		It("[test_id:47585] can change kubeletconfig and controller should adapt", Label(label.Reboot, label.Slow, label.Tier2, label.OpenShift), func() {
			...
			if len(targetedKC.OwnerReferences) == 1 {
				...
				updatePerformanceProfileFieldUnstructured(context.TODO(), ref.Name, tmScopeAnn, "metadata", "annotations", "kubeletconfig.experimental")
			}
