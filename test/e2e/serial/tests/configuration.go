...
var _ = Describe("[serial][disruptive] numaresources configuration management", Serial, Label("disruptive"), Label("feature:config"), func() {
	...
	Context("cluster has at least one suitable node", func() {
		...
		It("[test_id:75354][reboot_required][slow][unsched][schedrst] should be able to correctly identify topology manager policy without scheduler restarting", Label("reboot_required", "slow", "unsched", "schedrst"), Label("feature:schedattrwatch"), func(ctx context.Context) {
			...
			var performanceProfile ntov2.PerformanceProfile
			profiles := &ntov2.PerformanceProfileList{}
			err = fxt.Client.List(ctx, profiles)
			Expect(err).ToNot(HaveOccurred(), "unable to query performance profile")

			for _, profile := range profiles.Items {
				if profile.Kind == "PerformanceProfile" {
			...
			if numTargetedKCOwnerReferences == 1 {
				...
				if initialTopologyManagerPolicy != v1beta1.NoneTopologyManagerPolicy {
					...
					updatedProfile.Spec.NUMA = &ntov2.NUMA{
			...
			defer func() {
				...
				if initialTopologyManagerPolicy != kcObj.TopologyManagerPolicy {
					...
					if numTargetedKCOwnerReferences == 1 {
						...
						performanceProfile.Spec.NUMA = &ntov2.NUMA{
			...
			By("creating a Topology Affinity Error deployment and checking if the pod's status is pending")
			deployment := createDeploymentWithCustomCPUResources(fxt, ctx, "testdp", serialconfig.Config.SchedulerName, cpuResources)
			...
			Expect(err).ToNot(HaveOccurred(), "failed to get the pods list from the deployment")
...
func createDeploymentWithCustomCPUResources(fxt *e2efixture.Fixture, ctx context.Context, name, schedulerName, cpus string) *appsv1.Deployment {
	var err error
