...
var _ = Describe("[serial][disruptive][scheduler] numaresources workload unschedulable", Serial, Label("disruptive", "scheduler"), Label("feature:unsched"), func() {
	...
	Context("with zero suitable nodes", func() {
		It("[test_id:47615] a deployment with multiple guaranteed pods resources that doesn't fit at the NUMA level", Label(label.Tier2, "unsched"), func() {
			...
			// TODO: explain relationship with xload
			numaLevelFreeRes := corev1.ResourceList{
				corev1.ResourceCPU:    resource.MustParse("5"),
				corev1.ResourceMemory: resource.MustParse("5Gi"),
			}
			numaLevelFitRequiredRes := corev1.ResourceList{
				corev1.ResourceCPU:    resource.MustParse("2"),   // less than available
				corev1.ResourceMemory: resource.MustParse("2Gi"), // ditto
			}
			unfitRequiredRes := corev1.ResourceList{
				corev1.ResourceCPU:    resource.MustParse("7"),   // must be strictly more than available
				corev1.ResourceMemory: resource.MustParse("7Gi"), // ditto
			}

			By("padding all the nodes")
			err = padder.Nodes(len(nrts)).UntilAvailableIsResourceListPerZone(numaLevelFreeRes).Pad(time.Minute*2, e2epadder.PaddingOptions{
