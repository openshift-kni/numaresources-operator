		...
		When("adding taints to nodes in the target MCP", func() {
			...
			When("RTE pods are not running yet", func() {
				...
				BeforeEach(func(ctx context.Context) {
					...
					By("verifying RTE pods removal on all worker nodes")
					maxRetries := 10
					for step := range maxRetries {
						time.Sleep(10 * time.Second)

						allPodsGone := true
						for _, node := range workers {
							_, found := isRTEPodFoundOnNode(fxt.Client, ctx, node.Name)
							if found {
								klog.Infof("RTE pod still found on node %s (%d/%d)", node.Name, step+1, maxRetries)
								allPodsGone = false
								break
							}
						}
						...
						if allPodsGone {
							klog.Info("Confirmed: no RTE pods are running or terminating on any worker nodes")
							break
						}

						if step == maxRetries-1 {
							klog.Errorf("Timeout: RTE pods were not removed from all worker nodes after CR deletion")
						}
					}
