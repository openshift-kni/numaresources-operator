...
var _ = Describe("[serial][disruptive][rtetols] numaresources RTE tolerations support", Serial, Label("disruptive", "rtetols"), Label("feature:rtetols"), func() {
	...
	Context("cluster has at least one suitable node", func() {
		...
		When("[tier2] invalid tolerations are submitted ", Label("tier2"), func() {
			const unsupportedValue = "Unsupported value"
			It("should handle invalid field: operator", func(ctx context.Context) {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(isDegradedInternalError(updatedNropObj.Status.Conditions, unsupportedValue)).To(BeTrue(), "Condition not degraded as expected")
			...
			When("RTE pods are not running yet", func() {
				...
				AfterEach(func(ctx context.Context) {
					...
					if errors.IsNotFound(err) {
						...
						waitForMcpUpdate(fxt.Client, ctx, mcpsInfo, MachineCount, time.Now().String())
					} else {
						Eventually(func(g Gomega) {
							err := fxt.Client.Get(ctx, nroKey, nropNewObj)
							Expect(err).ToNot(HaveOccurred())
...
func waitForMcpUpdate(cli client.Client, ctx context.Context, mcpsInfo []mcpInfo, updateType MCPUpdateType, identifier string) {
	klog.InfoS("waitForMcpUpdate START", "ID", identifier)
	...
	klog.InfoS("waitForMcpUpdate END", "ID", identifier)
}
