...
var _ = Describe("[serial][disruptive][preemption] priority-based workload placement functionality", Serial, Label("disruptive", "scheduler"), Label("feature:preemption"), func() {
	...
	When("preemptionMode is set to Enabled", func() {
		BeforeEach(func(ctx context.Context) {
			initialNroSchedObj := &nropv1.NUMAResourcesScheduler{}
			nroSchedKey := objects.NROSchedObjectKey()
			Expect(fxt.Client.Get(ctx, nroSchedKey, initialNroSchedObj)).To(Succeed(), "cannot get %q in the cluster", nroSchedKey.String())

			if initialNroSchedObj.Spec.PreemptionMode == nil || *initialNroSchedObj.Spec.PreemptionMode != nropv1.PreemptionEnabled {
				By("setting preemption mode to Enabled")
				setPreemptionMode(ctx, fxt, nroSchedKey, ptr.To(nropv1.PreemptionEnabled))
				DeferCleanup(func(ctxA context.Context) {
					By("reverting preemption mode")
					setPreemptionMode(ctxA, fxt, nroSchedKey, initialNroSchedObj.Spec.PreemptionMode)
				})
			}
		})
...
func setPreemptionMode(ctx context.Context, fxt *e2efixture.Fixture, key client.ObjectKey, newMode *nropv1.PreemptionMode) {
	GinkgoHelper()

	e2efixture.By("setting preemption mode to %v", newMode)
	Eventually(func(g Gomega) {
		obj := &nropv1.NUMAResourcesScheduler{}
		g.Expect(fxt.Client.Get(ctx, key, obj)).To(Succeed())
		obj.Spec.PreemptionMode = newMode
		g.Expect(fxt.Client.Update(ctx, obj)).To(Succeed())
	}).WithTimeout(5 * time.Minute).WithPolling(10 * time.Second).Should(Succeed())

	e2efixture.By("verifying the scheduler object was updated properly with the new value %v", newMode)
	updatedSchedObj := &nropv1.NUMAResourcesScheduler{}
	Eventually(func(g Gomega) error {
		g.Expect(fxt.Client.Get(ctx, key, updatedSchedObj)).To(Succeed())
	...
	Expect(updatedSchedObj.Spec.PreemptionMode).To(Equal(newMode), "failed to update the schedulerName field,expected %v but found %v", newMode, updatedSchedObj.Spec.PreemptionMode)
}
