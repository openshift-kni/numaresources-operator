...
var _ = Describe("[Install] continuousIntegration", func() {
	...
	Context("with a running cluster with all the components", func() {
		It("[test_id:47574][tier1] should perform overall deployment and verify the condition is reported as available", func() {
			...
			if err != nil {
				logRTEPodsLogs(e2eclient.Client, e2eclient.K8sClient, context.TODO(), updatedNROObj, "RTE condition did not become available")
			}
			Expect(err).NotTo(HaveOccurred(), "RTE condition did not become available")
