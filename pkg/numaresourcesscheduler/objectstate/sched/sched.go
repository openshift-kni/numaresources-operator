...
type ExistingManifests struct {
	...
	DenyAllNetworkPolicyError   error
...
func (em ExistingManifests) State(mf schedmanifests.Manifests) []objectstate.ObjectState {
	return []objectstate.ObjectState{
		...
		{
			Existing: em.Existing.DenyAllNetworkPolicy,
			Error:    em.DenyAllNetworkPolicyError,
			Desired:  mf.DenyAllNetworkPolicy.DeepCopy(),
			Compare:  compare.Object,
			Merge:    merge.MetadataForUpdate,
		},
		{
...
func FromClient(ctx context.Context, cli client.Client, mf schedmanifests.Manifests) ExistingManifests {
	...
	if ret.DenyAllNetworkPolicyError = cli.Get(ctx, client.ObjectKeyFromObject(mf.DenyAllNetworkPolicy), np); ret.DenyAllNetworkPolicyError == nil {
		ret.Existing.DenyAllNetworkPolicy = np
	}
