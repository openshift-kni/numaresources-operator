...
func newUndiscoveredClusterResult() DiscoverResult {
...
func DiscoverCluster(ctx context.Context, platformName, platformVersion string) (DiscoverResult, error) {
	result := newUndiscoveredClusterResult()
	...
	if clusterPlatform == platform.Unknown {
		...
		return newUndiscoveredClusterResult(), err
	}
	...
	klog.InfoS("detected cluster", "platform", result.Platform, "short_version", result.ShortVersion, "long_version", result.LongVersion)
