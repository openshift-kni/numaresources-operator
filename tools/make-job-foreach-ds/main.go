...
func main() {
	...
	for _, ds := range dss {
		...
		job, err := MakeChrootJob(cj, &dsInstance)
		if err != nil {
			log.Printf("preparing the job definition for daemonset %s: %v", ds.String(), err)
			continue
		}
		data, err := yaml.Marshal(job)
		if err != nil {
			log.Printf("creating the job YAML to match daemonset %s: %v", ds.String(), err)
			continue
		}
		fmt.Println("---")
...
func MakeChrootJob(cj ChrootJob, ds *appsv1.DaemonSet) (*batchv1.Job, error) {
	namespace := ds.Namespace
	if cj.Namespace != "" {
	...
	if rteCnt == nil {
		return nil, fmt.Errorf("cannot find RTE container by name with name %s", rteupdate.MainContainerName)
	}
