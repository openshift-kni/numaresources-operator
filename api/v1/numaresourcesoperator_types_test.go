...
func TestNodeGroupConfigToString(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			expected: "PodsFingerprinting mode: Disabled InfoRefreshMode: Events InfoRefreshPeriod: {33s} InfoRefreshPause: Enabled NumaPlacement mode: Enabled Tolerations: [{Key:foo Operator: Value:1 Effect:NoSchedule TolerationSeconds:<nil>}]",
		},
	}
...
func TestNodeGroupToString(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			expected: "PoolName: pn MCPSelector: <missing> Config: PodsFingerprinting mode: Disabled InfoRefreshMode: Events InfoRefreshPeriod: {10s} InfoRefreshPause: Disabled NumaPlacement mode: Enabled Tolerations: [] Annotations: ann1=val1,ann2=val2",
		},
		{
			...
			expected: "PoolName: pn MCPSelector: <missing> Config: PodsFingerprinting mode: Disabled InfoRefreshMode: Events InfoRefreshPeriod: {10s} InfoRefreshPause: Disabled NumaPlacement mode: Enabled Tolerations: [] Annotations: ann1=val1,ann2=val2,ann3=val7,ann5=val4,ann9=valA",
		},
	}
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
