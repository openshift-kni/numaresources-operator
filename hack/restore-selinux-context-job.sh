...
# *** WARNING *** this is an example. You need to amend it to fit your cluster
# *** WARNING *** e.g. check and fix the container image to make sure it is accessible
    ...
    spec:
      ...
      containers:
        - name: selinux-context-restoration-cnt
          ...
          command: ["/usr/sbin/chroot", "/host", "/usr/sbin/restorecon", "-R", "-v", "/var/lib/kubelet/pod-resources"]
          securityContext:
            privileged: true
            runAsUser: 0
          volumeMounts:
