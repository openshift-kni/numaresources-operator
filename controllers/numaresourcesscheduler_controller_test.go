...
var _ = ginkgo.Describe("Test NUMAResourcesScheduler Reconcile", func() {
	...
	ginkgo.Context("with kubelet PodResourcesAPI listing active pods by default", func() {
		...
		ginkgo.When("kubelet fix is enabled", func() {
			fixedVersion, _ := platform.ParseVersion("4.16.49")
			unfixedVersion, _ := platform.ParseVersion("4.16.0")             // can't (and we must not even if we can) rewrite history
			futureFixedVersionZstream, _ := platform.ParseVersion("4.16.50") // we must never regress
			...
			ginkgo.It("should allow to update the informerMode to be Dedicated after an overridden default", func() {
				var err error
				nrs = testobjs.NewNUMAResourcesScheduler("numaresourcesscheduler", "some/url:latest", testSchedulerName, 11*time.Second)
				reconciler, err = NewFakeNUMAResourcesSchedulerReconciler(nrs)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())

				reconciler.PlatformInfo = platforminfo.PlatformInfo{Platform: platform.OpenShift, Version: fixedVersion}
