...
func (r *NUMAResourcesSchedulerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if req.Name != objectnames.DefaultNUMAResourcesSchedulerCrName {
		...
		return ctrl.Result{}, r.updateStatus(ctx, initialStatus, instance, status.ConditionDegraded, conditionTypeIncorrectNUMAResourcesSchedulerResourceName, message)
	}
	...
	if err := r.updateStatus(ctx, initialStatus, instance, condition, reason, messageFromError(err)); err != nil {
