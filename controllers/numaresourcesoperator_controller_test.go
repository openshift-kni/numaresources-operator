...
var _ = Describe("Test NUMAResourcesOperator Reconcile", func() {
	...
	Context("with correct NRO and more than one NodeGroup each with different pool specifier", func() {
		It("should create RTE daemonset and reflect status in NodeGroupStatus", func(ctx context.Context) {
			mcpName := "test1"
			...
			Expect(nroUpdated.Status.NodeGroups[0].PoolName).To(Equal(mcp.Name))
