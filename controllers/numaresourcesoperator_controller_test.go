...
var _ = Describe("Test NUMAResourcesOperator Reconcile", func() {
	...
	Context("with NodeGroupConfig", func() {
		...
		When("with single NodeGroup", func() {
		...
		When("with multiple NodeGroups", func() {
			When("node groups have different config", func() {
				It("should create all RTE daemonsets and report the status respectively per node group", func(ctx context.Context) {
					mcp1Name := "test1"
					label1 := map[string]string{
						mcp1Name: mcp1Name,
					}
					labelSelector1 := &metav1.LabelSelector{MatchLabels: label1}

					mcp2Name := "test2"
					label2 := map[string]string{
						mcp2Name: mcp2Name,
					}
					labelSelector2 := &metav1.LabelSelector{MatchLabels: label2}

					mcp1 := testobjs.NewMachineConfigPool(mcp1Name, label1, labelSelector1, labelSelector1)
					mcp2 := testobjs.NewMachineConfigPool(mcp2Name, label2, labelSelector2, labelSelector2)
					nro := testobjs.NewNUMAResourcesOperator(objectnames.DefaultNUMAResourcesOperatorCrName, []*metav1.LabelSelector{labelSelector1, labelSelector2})
					...
					nro.Spec.NodeGroups[0].Config = conf1
					nro.Spec.NodeGroups[1].Config = conf2
					...
					Expect(nroUpdated.Status.DaemonSets[0].Name).To(Equal(objectnames.GetComponentName(nro.Name, mcp1.Name)))
					...
					Expect(nroUpdated.Status.DaemonSets[1].Name).To(Equal(objectnames.GetComponentName(nro.Name, mcp2.Name)))
